	...
	useEffect(
		() => {
			const timer = setTimeout(() => {
				seconds > 0 ? setSeconds(seconds - 1) : clearTimeout(timer);
			}, 1000);
		},
		[ seconds ]
	);
		...
		<div className="container">
			...
			<section className="buttons">
				<div className="homeButtons">
					...
					<button className="homeButtons__touchdown" onClick={() => setScore(score + 6)}>
						Home Touchdown
					</button>
					<button className="homeButtons__fieldGoal" onClick={() => setScore(score + 3)}>
						Home Field Goal
					</button>
				</div>
